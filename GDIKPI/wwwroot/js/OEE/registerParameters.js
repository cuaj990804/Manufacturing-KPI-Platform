...
$(document).on("change", "#metricName, #category", function () {
    let metricName = $("#metricName").val();
    let category = $("#category").val();

    if (metricName && category) {
        $.ajax({
            url: '/api/OEEApi/OEEPercentage',
            type: 'GET',
...
$(document).on("submit", "#AddOEEParamForm", function (e) {
    e.preventDefault();
    ...
    $.ajax({
        url: '/api/OEEApi/UpdateParameters',
        type: 'POST',
        data: {
